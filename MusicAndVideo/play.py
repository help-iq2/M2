...
@Client.on_message(filters.command(["بدء_من"], prefixes=f"{HNDLR}"))
...
async def skip(client, m: Message):
    ...
    if len(m.command) < 2:
        ...
        if op == 0:
            ...
            await m.reply(                f"**⏭ Melewati pemutaran** \n**🎧 Sekarang memutar** - [{op[0]}]({op[1]}) | `{op[2]}`",                disable_web_page_preview=True,            )
    else:
...
@Client.on_message(filters.command(["اعاده"], prefixes=f"{HNDLR}"))
@authorized_users_only
